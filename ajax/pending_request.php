...
if($action == 'pending') 
{
    ...
    if($row) 
	{
		$notificaions = mysqli_query($mysqli,"insert into notifications (post_id,user_id,post_user_id,type,date,status) values ('$id','$follow_id','$user_id','accepted',NOW(),'unseen')");
		$updateQuery = mysqli_query($mysqli,"update follower set status ='accepted' where id='$id'");
    }
    
    
    $vin = send_notification($user_id,$follow_id,"accepted");
